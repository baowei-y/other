...
MASTER_CTRL(){
  ...
  if PERIOD_CHECK ;then
    for f in `find $final_dir -type f 2> /dev/null`;do
      DEL_FUC $f $5 $6
    done
  fi
  rm -rf $pid_file
...
MASTER_CTRL $put_hdfs_list $put_retry_list $thread_script $threads $del_str $retention_day
